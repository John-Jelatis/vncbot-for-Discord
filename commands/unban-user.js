...
const invalidUsage = new Discord.MessageEmbed()
	.setTitle('Unban user')
	.setColor('#ff0000')
	.setDescription('You need to mention a user to unban from the bot');

const alreadyDone = new Discord.MessageEmbed()
	.setTitle('Unbanned user')
	.setColor('#ff0000')
	.setDescription('User already has been unbanned!');

module.exports = {
	'name': 'unban-user',
	'aliases': [ 'unban', 'unblock', 'pardon' ],
	'desc': 'Unban users from using the bot.',
	...
	'run': async function(msg, args) {
		...
		svrInf.banned.splice(svrInf.banned.indexOf(user.id), 1);

		const userBanned = new Discord.MessageEmbed()
			.setTitle('User unbanned')
			.setColor('#99ff00')
			.setDescription('Unbanned `' + user.username + '#' + user.discriminator + '` from using the bot.');
